...
class Game:
    ...
    def find_safe_teleport_positions(self, pacman_pos):
        ...
        for distance in range(min_distance, max_distance + 1):
            ...
            # Thêm các vị trí xung quanh theo hướng vuông góc
            if dir_x != 0:  # Nếu di chuyển theo trục X (LEFT/RIGHT)
            ...
            else:  # Nếu di chuyển theo trục Y (UP/DOWN)
                positions_to_check.extend([
                    (base_x + 1, base_y),
                    (base_x - 1, base_y),
                    (base_x + 2, base_y),
        ...
        # Thêm thêm vị trí ngẫu nhiên trong hướng đã chọn
        for _ in range(50):
